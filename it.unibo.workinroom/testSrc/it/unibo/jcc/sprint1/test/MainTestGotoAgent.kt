...
//	delay(5000)
	...
	GlobalScope.launch {
	    println(" %%%%%%% MainTestGotoAgent starting ctxGotoagent")
		it.unibo.ctxGotoagent.main()
	}
	delay(5000)		//give the time to start
	worker = sysUtil.getActor("gotobehavior")
//	println(" %%%%%%% TestRobotmind getActors resource=${worker}")

	// Send goal
	println(" %%%%%%% MainTestGotoAgent goto(pantry)")
	MsgUtil.sendMsg("setGoal","setGoal(goto(pantry))",worker!!)
	...
	// Send goal
	println(" %%%%%%% MainTestGotoAgent goto(fridge)")
	MsgUtil.sendMsg("setGoal","setGoal(goto(fridge))",worker!!)
	
	delay(3000)
	
	println(" %%%%%%% MainTestGotoAgent suspend")
	MsgUtil.sendMsg("suspend","suspend",worker!!)
	
	delay(4000)
	
	println(" %%%%%%% MainTestGotoAgent resume")
